...
class BaseTrain(metaclass=abc.ABCMeta):
    '''
    the fifth version of BaseTrain class
    the net load & save system was optimized
    the param load & save system was optimized
    ...
    def _cfginit(self, cfgfile):
        ...
        self.checkPoint = self.args.checkpoint if self.args.checkpoint else self.config.getint(self.ID, "CHECKPOINT")

        self.threshold = self.args.threshold if self.args.threshold else self.config.getfloat(self.ID, "THRESHOLD")
    ...
    def saveBatch(self):
        if self.formalTrain:
            try:
                self._log(i=self.i, j=self.j, k=self.k, loss=self.loss.item())

...
class BaseTrain(metaclass=abc.ABCMeta):
    def __init__(self, netName, net, cfgfile, index=0, basepath=BASEPATH):
        ...
        self.netID = f"{self.netName}_{self.index}"

        self._cfginit(cfgfile)
        self.subSaveDir = os.path.join(self.saveDir, f"{self.netName}")
        self.subNetSaveDir = os.path.join(self.subSaveDir, f"{self.netID}")
    ...
    def _cfginit(self, cfgfile):
        ...
        self.save = self.args.save if (self.args.save != None) else config.getint(self.netName, "SAVE")
    ...
    def _moduleinit(self, module):
        if module:
            self.netfile = os.path.join(self.subSaveDir, f"{self.netID}.pt")
            self.scriptfile = os.path.join(self.subSaveDir, f"{self.netID}.script")
            self.netfile_backup = os.path.join(
                self.subSaveDir, f"{self.netID}_backup.pt")
    ...
    def _loginit(self):
        self.logFile = os.path.join(self.subSaveDir, f"{self.netID}.log")
        self.logFileTest = os.path.join(
            self.subSaveDir, f"{self.netID}_test.log")
        self.logFile_backup = os.path.join(
            self.subSaveDir, f"{self.netID}_backup.log")
        self.logFileTest_backup = os.path.join(
            self.subSaveDir, f"{self.netID}_test_backup.log")
    ...
    def plot(self, *args, isTest=False):
        for item in args:
            plotName = f"plot_{self.netID}_{item}.png"  # jpg in linux wrong
